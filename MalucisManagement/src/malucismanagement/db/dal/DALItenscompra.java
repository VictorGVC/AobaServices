...
public class DALItenscompra {
    ...
    public boolean alterar(Itenscompra it) throws SQLException {
        ...
        String sql = "UPDATE itenscompra SET "
                + "mar_cod =#1, pro_cod ='#2', it_quantidade =#3, it_peco =#4 WHERE com_cod="+it.getCom_cod();
    ...
    public List<Itenscompra> getItenscompra(){
        ...
        try {
            while(rs.next()){
                lista.add(new Itenscompra(Integer.parseInt(rs.getString("com_cod")),Integer.parseInt(rs.getString("it_quantidade")),
                        Integer.parseInt(rs.getString("mar_cod")),rs.getString("pro_cod"),Double.parseDouble(rs.getString("it_preco"))));
                }
        } catch (Exception e) {
